...
namespace MCS.Controllers
{
    ...
    public class ApiTaskController : ApiController, IJqOneTable
    {
        [System.Web.Http.HttpPost]
        public JObject Count(JObject obj)
        {
            BaseTask sqlCreator = new BaseTask();
            DAO dao = new DAO();
        ...
        public JObject Insert(JObject obj)
        {
            ...
            JProperty guid = new JProperty("GUID", "");
            obj.Add(guid);
            var sqlParms = sqlCreator.CreateParameterAry(parm);
            
            string checkRepeat = sqlCreator.CheckRepeat();
            dao.AddExecuteItem(checkRepeat, sqlParms);
        ...
        public JObject Copy(JObject obj)
        {
            BaseTask sqlCreator = new BaseTask();
            DAO dao = new DAO();
            dynamic returnMsg = new JObject();
            ...
            var sqlStruuid = sqlCreator.GetUUID();
            dao.AddExecuteItem(sqlStruuid, sqlParms);
            var Taskuuid = dao.Query().Tables[0].Rows[0].ItemArray[0].ToString();
            var sqlTaskStr = $@"INSERT INTO BASE_TASK (
            GUID,TASK_ID,TASK_NAME,REMARK,INSERT_USER,INSERT_TIME) 
            VALUES ('{Taskuuid}',@TASK_ID,@TASK_NAME,@REMARK,system_user(),now());";
            dao.AddExecuteItem(sqlTaskStr, sqlParms);

            //新增任務流程編輯
            var sqlTaskSubtask = $@"INSERT INTO BASE_TASK_SUBTASK
            (GUID,TASK_GUID,SUBTASK_GUID,AGVGROUP_GUID,
            PROGRESS,POSITIONCODEPATH,
            START_FLOOR,TARGET_FLOOR,
            DIRECTION,API,
            INSERT_USER,INSERT_TIME,
            ENABLE,REMARK,BOOKING)
            SELECT 
            uuid(),'{Taskuuid}',SUBTASK_GUID,AGVGROUP_GUID,
            PROGRESS,POSITIONCODEPATH,            
            START_FLOOR,TARGET_FLOOR,
            DIRECTION,API,
            '@UPDATE_USER',now(),
            ENABLE,REMARK,BOOKING
            FROM BASE_TASK_SUBTASK
            WHERE TASK_GUID = @GUID ";
            dao.AddExecuteItem(sqlTaskSubtask, sqlParms);
