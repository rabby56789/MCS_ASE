...
namespace MCS.Models
{
    ...
    /// </summary>
    public class TaskList : ISqlCreator
    {
        ...
        public string Search(JObject parm, bool getCount)
        {
            ...
            {
                sqlStr = string.Format(@"SELECT *,CASE D.TASKSTATUS WHEN '0' THEN '未建立' WHEN '1' THEN '待執行' WHEN '2' THEN '執行中' WHEN '3' THEN '完成' WHEN '4' THEN '失敗' WHEN '5' THEN '建立失敗' END AS STATUS FROM {0} D ", MasterTable);
            ...
            {
                sqlStr += "AND D.TASKSTATUS=@TASKSTATUS ";
            }
            //任務ID
            if (!string.IsNullOrEmpty((string)conditions.TASKCODE))
            {
                sqlStr += "AND D.TASKCODE LIKE CONCAT(@TASKCODE , '%') ";
            }
            //AGV
            if (!string.IsNullOrEmpty((string)conditions.AGVCODE))
            {
                sqlStr += "AND D.AGVCODE LIKE CONCAT(@AGVCODE , '%') ";
            }
            //MATERIALLOT
            if (!string.IsNullOrEmpty((string)conditions.MATERIALLOT))
            {
                sqlStr += "AND D.MATERIALLOT LIKE CONCAT(@MATERIALLOT , '%') ";
            }
            //建立時間-起
        ...
        public string Insert(string uuid, JObject parm)
        {
            ...
            if (!string.IsNullOrEmpty((string)conditions.POSITIONCODE_GUID))
            {
                sqlStr += "POSITIONCODE_GUID,";
                sqlValue += "@POSITIONCODE_GUID,";
            }
