...
namespace MCS.Models
{
    ...
    /// </summary>
    public class AgvGroup : ISqlCreator
    {
        public string MasterTable { get { return "BASE_AGVGROUP"; } }
        public string HistoryTable { get { return "BASE_AGVGROUP_HISTORY"; } }
        ...
        public string Search(JObject parm, bool getCount)
        {
            ...
            {
                sqlStr = string.Format(@"SELECT D.GUID,D.AGVGROUP_ID,D.AGVGROUP_NAME,D.REMARK,D.INSERT_TIME FROM {0} D ", MasterTable);
            }

            sqlStr += "WHERE D.ENABLE = 1 ";

            //代碼
            if (!string.IsNullOrEmpty((string)conditions.AGVGROUP_ID))
            {
                sqlStr += "AND D.AGVGROUP_ID LIKE CONCAT(@AGVGROUP_ID, '%') ";
            }
            //名稱
            if (!string.IsNullOrEmpty((string)conditions.AGVGROUP_NAME))
            {
                sqlStr += "AND D.AGVGROUP_NAME LIKE CONCAT(@AGVGROUP_NAME , '%') ";
            }
            //建立時間-起
        ...
        {
            string sqlStr = string.Format(@"SELECT D.GUID,D.AGVGROUP_ID,D.AGVGROUP_NAME,D.REMARK FROM {0} D ", MasterTable);
        ...
        {
            string sqlStr = string.Format(@"INSERT INTO {0} (GUID,AGVGROUP_ID,AGVGROUP_NAME,REMARK,INSERT_USER,INSERT_TIME) ", MasterTable);
            sqlStr += $@"VALUES ('{uuid}',@AGVGROUP_ID,@AGVGROUP_NAME,@REMARK,@INSERT_USER,now());";
            sqlStr += string.Format(@"INSERT INTO {0}(GUID,AGVGROUP_GUID,AGVGROUP_ID,`AGVGROUP_NAME`,INSERT_USER,INSERT_TIME,UPDATE_USER,UPDATE_TIME,`ENABLE`,REMARK)", HistoryTable);
            sqlStr += string.Format(@"SELECT UUID(), GUID,AGVGROUP_ID,`AGVGROUP_NAME`,INSERT_USER,INSERT_TIME,UPDATE_USER,UPDATE_TIME,`ENABLE`,REMARK FROM {0} ", MasterTable);
        ...
        public string Update()
        {
            string sqlStr = string.Format(@"UPDATE {0} SET 
                AGVGROUP_ID = @GROUP_ID,
                AGVGROUP_NAME = @GROUP_NAME,
                REMARK = @REMARK,
