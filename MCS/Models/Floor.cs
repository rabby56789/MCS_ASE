...
namespace MCS.Models
{
    ...
    /// </summary>
    public class Floor : ISqlCreator
    {
        public string MasterTable { get { return "BASE_FLOOR"; } }
        public string HistoryTable { get { return "BASE_FLOOR_HISTORY"; } }
        public string SubTable { get { return "BASE_FACTORY"; } }
        /// <summary>
        ...
        public string Search(JObject parm, bool getCount)
        {
            ...
            if (getCount)
            {
                sqlStr = string.Format(@"SELECT COUNT(*) `Count` FROM {0} D , {1} F ", MasterTable , SubTable);
            }
            else
            {
                sqlStr = string.Format(@"SELECT D.GUID,D.ID,D.NAME,D.REMARK,D.INSERT_TIME,F.NAME AS FACTORY_NAME FROM {0} D , {1} F ", MasterTable , SubTable);
            ...
            sqlStr += "WHERE D.ENABLE = 1 AND D.FACTORY_GUID=F.GUID ";

            //廠GUID
            if (!string.IsNullOrEmpty((string)conditions.FACTORY_GUID))
            {
                sqlStr += "AND D.FACTORY_GUID LIKE CONCAT(@FACTORY_GUID, '%') ";
            }
        ...
        {
            string sqlStr = string.Format(@"SELECT GUID AS 'Key', ID , NAME AS 'Value' FROM {0} D ", SubTable);
            sqlStr += "WHERE D.ENABLE = 1 ";
            return sqlStr;
        }
        public string Insert(string uuid)
        {
            string sqlStr = string.Format(@"INSERT INTO {0} (GUID,FACTORY_GUID,ID,NAME,REMARK,INSERT_USER,INSERT_TIME) ", MasterTable);
            sqlStr += $@"VALUES ('{uuid}',@FACTORY_GUID,@ID,@NAME,@REMARK,@INSERT_USER,now());";
            sqlStr += string.Format(@"INSERT INTO {0}(GUID,FLOOR_GUID,FACTORY_GUID,ID,`NAME`,INSERT_USER,INSERT_TIME,UPDATE_USER,UPDATE_TIME,`ENABLE`,REMARK)", HistoryTable);
            sqlStr += string.Format(@"SELECT UUID(), GUID,FACTORY_GUID,ID,`NAME`,INSERT_USER,INSERT_TIME,UPDATE_USER,UPDATE_TIME,`ENABLE`,REMARK FROM {0} ", MasterTable);
