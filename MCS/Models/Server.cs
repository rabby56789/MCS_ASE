...
namespace MCS.Models
{
    ...
    /// </summary>
    public class Server : ISqlCreator
    {
        public string MasterTable { get { return "BASE_SERVER"; } }
        public string HistoryTable { get { return "BASE_SERVER_HISTORY"; } }
        ...
        public string Search(JObject parm, bool getCount)
        {
            ...
            {
                sqlStr = string.Format(@"SELECT * FROM {0} D ", MasterTable);
            }

            sqlStr += "WHERE D.ENABLE = 1 ";

            //代碼
            if (!string.IsNullOrEmpty((string)conditions.SERVER_IP))
            {
                sqlStr += "AND D.SERVER_IP LIKE CONCAT(@SERVER_IP, '%') ";
            }
            //名稱
            if (!string.IsNullOrEmpty((string)conditions.SERVER_NAME))
            {
                sqlStr += "AND D.SERVER_NAME LIKE CONCAT(@SERVER_NAME , '%') ";
            }
            //建立時間-起
        ...
        {
            string sqlStr = string.Format(@"INSERT INTO {0} (GUID,SERVER_IP,SERVER_NAME,SERVER_PORT,SERVER_FUNCTION,URL,SERVER_TYPE,REMARK,INSERT_USER,INSERT_TIME) ", MasterTable);
            sqlStr += $@"VALUES ('{uuid}',@SERVER_IP,@SERVER_NAME,@SERVER_PORT,@SERVER_FUNCTION,@URL,@SERVER_TYPE,@REMARK,@INSERT_USER,now());";
            sqlStr += string.Format(@"INSERT INTO {0}(GUID,SERVER_GUID,SERVER_IP,SERVER_NAME,SERVER_PORT,SERVER_FUNCTION,URL,SERVER_TYPE,INSERT_USER,INSERT_TIME,UPDATE_USER,UPDATE_TIME,`ENABLE`,REMARK)", HistoryTable);
            sqlStr += string.Format(@"SELECT UUID(), GUID,SERVER_IP,SERVER_NAME,SERVER_PORT,SERVER_FUNCTION,URL,SERVER_TYPE,INSERT_USER,INSERT_TIME,UPDATE_USER,UPDATE_TIME,`ENABLE`,REMARK FROM {0} ", MasterTable);
